...
class SiglipTimeseriesConfig:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.image_size = waveform_size
        self.attention_dropout = attention_dropout
        self.layer_norm_eps = layer_norm_eps
        self.num_image_tokens = num_image_tokens


class SiglipTimeseriesEmbeddings(nn.Module):
    def __init__(self, config: SiglipTimeseriesConfig):
        super().__init__()
        self.config = config
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        _, _, height = pixel_values.shape # [Batch_Size, Channels, Height]
...
class SiglipTimeseriesTransformer(nn.Module):
    def __init__(self, config: SiglipTimeseriesConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size

        self.embeddings = SiglipTimeseriesEmbeddings(config)
...
class SiglipTimeseriesModel(nn.Module):

    def __init__(self, config: SiglipTimeseriesConfig):
        super().__init__()
        self.config = config
        self.Timeseries_model = SiglipTimeseriesTransformer(config)

    def forward(self, pixel_values) -> Tuple:
        # [Batch_Size, Channels, Height, Width] -> [Batch_Size, Num_Patches, Embed_Dim]
        return self.Timeseries_model(pixel_values=pixel_values) 
